  ...
  return `
    <div style="font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,sans-serif;max-width:520px;margin:0 auto;background:#ffffff;">
      <div style="background:#FF8C00;padding:24px 20px;text-align:center;border-radius:12px 12px 0 0;">
        <h1 style="margin:0;color:#ffffff;font-size:24px;font-weight:800;letter-spacing:-0.5px;">DonutDash&trade;</h1>
      </div>
...
export async function notifyNewSignup(input: {
  ...
  if (!SIGNUP_ALERT_ROLES.has(input.role)) return false
  ...
  const html = `
    <div style="font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,sans-serif;max-width:520px;margin:0 auto;background:#ffffff;">
      <div style="background:#FF8C00;padding:24px 20px;text-align:center;border-radius:12px 12px 0 0;">
        <h1 style="margin:0;color:#ffffff;font-size:22px;font-weight:800;">New DonutDash&trade; signup</h1>
      </div>
      <div style="padding:24px 20px;border:1px solid #eee;border-top:none;border-radius:0 0 12px 12px;">
...
export async function notifyAdmins(message: string, emailSubject?: string, emailHtml?: string) {
  ...
  if (emails && emails.length > 0) {
    ...
    const html = emailHtml || `<div style="font-family:sans-serif;padding:20px;"><h2 style="color:#FF8C00;">DonutDash&trade; Order Alert</h2><pre style="font-size:14px;line-height:1.6;white-space:pre-wrap;">${message}</pre></div>`
