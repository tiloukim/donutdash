...
async function main() {
  const { data: shops } = await supabase.from('dd_shops').select('id, name, address').ilike('name', 'high street donuts')
  ...
  url.searchParams.set('pitch', '1.59')
  url.searchParams.set('panoid', 'Gt8E6nKTU3xGyf2ui1JCGA')
  url.searchParams.set('yaw', '44.18')
