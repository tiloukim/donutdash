/**
 * One-off: set Eagle Nest Donut Shop (1744 US-259, Diana, TX 75640) image
 * + banner from the Street View photo in
 * https://maps.app.goo.gl/Z4JcXxeteqHC4LKs9
 *
 *   npx tsx scripts/update-eagle-nest.ts
 */
...
const STREET_VIEW = {
  pano: 'qpUrDgNaTMNGSBi8j6jeug',
  heading: 211.7767701545019,
  pitch: 1.3317407307661284,
}

// Shop to update
const SHOP_ADDRESS_HINT = 'US-259' // or "1744"
const SHOP_NAME_HINT = 'eagle nest'
...
async function main() {
  // Prefer name match (more specific than address fragment)
  const { data: shops } = await supabase
    .from('dd_shops')
    .select('id, name, slug, address, city')
    .ilike('name', `%${SHOP_NAME_HINT}%`)

  if (!shops || shops.length === 0) {
    console.error(`No shop found matching "${SHOP_NAME_HINT}". You may need to add it to the database first.`)
    process.exit(1)
  }
  if (shops.length > 1) {
