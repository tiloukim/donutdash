...
async function main() {
  const { data: shops } = await supabase.from('dd_shops').select('id, name, address').ilike('name', 'donut palace').ilike('address', '%5th%')
  ...
  url.searchParams.set('pitch', '3.39')
  url.searchParams.set('panoid', 'a2N9_sOu5DZUITMDq1gUJQ')
  url.searchParams.set('yaw', '215.95')
