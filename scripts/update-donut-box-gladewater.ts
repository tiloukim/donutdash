...
const PHOTO_URL = 'https://lh3.googleusercontent.com/gpms-cs-s/ABJJf51ExADVgXXYg3W2AwuWSj2hfAQabHFIfZ3Lwm6Q9mnsHzRtqWYCElkwp5TdPfGtu9rnCxsY4Ms_WJrQgu7WEm-gsbiyQIYNPtrPrqqahD-XJ8dOP6SKDBhjLHxJ1TBpX4lgCaX18A=s1360-w1360-h1020-rw'

async function main() {
  const { data: shops } = await supabase.from('dd_shops').select('id, name, address, city').ilike('name', 'donut box').ilike('city', '%gladewater%')
  if (!shops?.length) { console.error('Not found'); process.exit(1) }
  const shop = shops[0]
  console.log(`Found: ${shop.name} — ${shop.address}, ${shop.city} (${shop.id})`)
