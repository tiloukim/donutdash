...
async function main() {
  const { data: shops } = await supabase.from('dd_shops').select('id, name, address').ilike('name', 'the donut palace').ilike('address', '%loop 323%')
  ...
  url.searchParams.set('pitch', '0.73')
  url.searchParams.set('panoid', 'qe1Umv6njBmW5V8UXBgXSg')
  url.searchParams.set('yaw', '157.06')
