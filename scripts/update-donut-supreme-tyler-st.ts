...
async function main() {
  // List all Donut Supreme in Athens to find the right one
  const { data: shops } = await supabase.from('dd_shops').select('id, name, address, city').ilike('name', 'donut supreme').ilike('city', '%athens%')
  if (!shops?.length) { console.error('Not found'); process.exit(1) }
  console.log('All Donut Supreme in Athens:')
  shops.forEach(s => console.log(`  ${s.name} — ${s.address} (${s.id})`))

  const shop = shops.find(s => s.address.toLowerCase().includes('tyler')) || shops.find(s => s.address.includes('1414'))
  if (!shop) { console.error('Could not find 1414 East Tyler Street'); process.exit(1) }
  ...
  url.searchParams.set('pitch', '-0.57')
  url.searchParams.set('panoid', 'IlFxWphTVTgRh8imuTg_qQ')
  url.searchParams.set('yaw', '115.34')
