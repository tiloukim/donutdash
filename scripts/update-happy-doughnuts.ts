...
async function main() {
  const { data: shops } = await supabase.from('dd_shops').select('id, name, address').ilike('name', 'happy doughnuts')
  ...
  url.searchParams.set('pitch', '3.43')
  url.searchParams.set('panoid', 'HBtwBa6jTvoxzuEcj534MQ')
  url.searchParams.set('yaw', '60.90')
