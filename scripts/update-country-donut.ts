...
const PANO = '-9Niz3HGOmeYjd4rDk10Bw'
const HEADING = 93.97933
const PITCH = 0
...
async function main() {
  const { data: shops } = await supabase
    ...
    .ilike('name', '%country donut%')
    .ilike('city', '%tyler%')

  if (!shops?.length) { console.error('No Country Donut in Tyler found'); process.exit(1) }

  console.log(`Found: ${shops[0].name} — ${shops[0].address} (${shops[0].id})`)
  ...
  url.searchParams.set('panoid', PANO)
  ...
  for (const folder of ['logos', 'banners'] as const) {
    ...
    if (error) { console.error(`${folder} upload:`, error.message); continue }
