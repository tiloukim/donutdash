...
async function main() {
  const { data: shops } = await supabase.from('dd_shops').select('id, name, address, city').ilike('name', 'donut palace').ilike('city', '%van%')
  ...
  url.searchParams.set('panoid', 'CEXDa5qR06fm_V55W6xTxA')
  url.searchParams.set('yaw', '358.79')
