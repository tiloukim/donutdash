...
async function main() {
  const { data: shops } = await supabase.from('dd_shops').select('id, name, address').ilike('name', 'donut palace').ilike('address', '%271%')
  ...
  url.searchParams.set('pitch', '6.09')
  url.searchParams.set('panoid', 'a-G9onUqBLVOLnZ6MJb3Rw')
  url.searchParams.set('yaw', '20.19')
