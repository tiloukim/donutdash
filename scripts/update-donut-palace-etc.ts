...
async function main() {
  const { data: shops } = await supabase.from('dd_shops').select('id, name, address, city').ilike('name', '%donut palace etc%')
  ...
  url.searchParams.set('pitch', '7.39')
  url.searchParams.set('panoid', 'JQnNsT2RnbQuG-mR6VxuOQ')
  url.searchParams.set('yaw', '46.35')
