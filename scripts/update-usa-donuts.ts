...
async function main() {
  const { data: shops } = await supabase.from('dd_shops').select('id, name, address').ilike('name', 'usa donuts')
  ...
  url.searchParams.set('pitch', '8.27')
  url.searchParams.set('panoid', '6Vi0OGm7TTwCS8KvTcqPPg')
  url.searchParams.set('yaw', '85.67')
