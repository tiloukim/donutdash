...
async function main() {
  const { data: shops } = await supabase.from('dd_shops').select('id, name, address, city').ilike('name', 'donut box').ilike('city', '%liberty%')
  ...
  url.searchParams.set('pitch', '-1.12')
  url.searchParams.set('panoid', 'FAQQGtzrYXDheQLQ-JuWLg')
  url.searchParams.set('yaw', '182.85')
