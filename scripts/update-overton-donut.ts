...
async function main() {
  const { data: shops } = await supabase.from('dd_shops').select('id, name, address').ilike('name', '%overton donut%')
  ...
  url.searchParams.set('panoid', 'fZAHFVxsP_u0Sy_8Ohf4Ew')
  url.searchParams.set('yaw', '271.87')
