...
async function main() {
  const { data: shops } = await supabase.from('dd_shops').select('id, name, address').ilike('name', '%lindsey park%')
  ...
  url.searchParams.set('pitch', '-5.04')
  url.searchParams.set('panoid', 'FdYHgwU4O2fcQ3_pZ4n0Mw')
  url.searchParams.set('yaw', '162.37')
