...
async function main() {
  const { data: shops } = await supabase.from('dd_shops').select('id, name, address').ilike('name', 'jimmy%donuts')
  ...
  url.searchParams.set('pitch', '1.33')
  url.searchParams.set('panoid', 'wB7R90TSxrxTyRXKcsR3JA')
  url.searchParams.set('yaw', '18.56')
