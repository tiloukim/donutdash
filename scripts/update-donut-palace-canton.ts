...
async function main() {
  const { data: shops } = await supabase.from('dd_shops').select('id, name, address, city').ilike('name', 'donut palace').ilike('city', '%canton%')
  ...
  url.searchParams.set('pitch', '2.19')
  url.searchParams.set('panoid', 'O5tjDCdEQamYtzWo6gBRVg')
  url.searchParams.set('yaw', '6.63')
