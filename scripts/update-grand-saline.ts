...
async function main() {
  const { data: shops } = await supabase.from('dd_shops').select('id, name, address').ilike('name', 'grand saline donuts')
  ...
  url.searchParams.set('pitch', '0.62')
  url.searchParams.set('panoid', 'duMwSpI9CETN5RyCHwZJ-w')
  url.searchParams.set('yaw', '0.33')
