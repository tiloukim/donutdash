...
async function main() {
  const { data: shops } = await supabase.from('dd_shops').select('id, name, address').ilike('name', 'star donuts').ilike('address', '%broadway%')
  ...
  url.searchParams.set('pitch', '6.55')
  url.searchParams.set('panoid', 'VSKYYEvUvksMyCC6tUTiug')
  url.searchParams.set('yaw', '313.57')
