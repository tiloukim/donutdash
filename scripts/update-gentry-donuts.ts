...
async function main() {
  const { data: shops } = await supabase.from('dd_shops').select('id, name, address').ilike('name', '%gentry donut%')
  ...
  url.searchParams.set('pitch', '-3.44')
  url.searchParams.set('panoid', 'XkyLJsk2Oo5JH3REaL5SjQ')
  url.searchParams.set('yaw', '187.50')
