...
async function main() {
  const { data: shops } = await supabase.from('dd_shops').select('id, name, address').ilike('name', 'adan donuts')
  ...
  url.searchParams.set('pitch', '-11.23')
  url.searchParams.set('panoid', 'Z0lAjiuPlxPV8TRMBppZbw')
  url.searchParams.set('yaw', '275.17')
