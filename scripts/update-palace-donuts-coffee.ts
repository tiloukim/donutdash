...
async function main() {
  const { data: shops } = await supabase.from('dd_shops').select('id, name, address').ilike('name', '%palace donuts%coffee%')
  ...
  url.searchParams.set('pitch', '0.18')
  url.searchParams.set('panoid', 'HLYOA7YExx9FjT1av0M2jQ')
  url.searchParams.set('yaw', '352.03')
