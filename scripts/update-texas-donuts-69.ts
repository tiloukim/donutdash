...
async function main() {
  const { data: shops } = await supabase.from('dd_shops').select('id, name, address').ilike('name', 'texas donuts').ilike('address', '%69%')
  ...
  url.searchParams.set('pitch', '13.89')
  url.searchParams.set('panoid', 'FKoA5hY9HzB-t2T-Xu4Lrg')
  url.searchParams.set('yaw', '262.06')
