...
const PHOTO_URL = 'https://lh3.googleusercontent.com/gps-cs-s/APNQkAFm4aGZsDFJwkBH2WdotGmgfzOq8j6oSpVd-4qmo7ROqKcmjz4ddcqoOzDtRyRhWgEKzvxPUNvPNlLBHLU9OZsBcM-Wh-ZG7CLfAConZvUMMgbM_FxcGjZdINf4Zd4fMDUEMpcP5vZRxKo=s1360-w1360-h1020-rw'

async function main() {
  const { data: shops } = await supabase.from('dd_shops').select('id, name, address').ilike('name', 'donut express').ilike('address', '%rusk%')
