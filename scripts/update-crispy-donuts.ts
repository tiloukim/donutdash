...
async function main() {
  const { data: shops } = await supabase.from('dd_shops').select('id, name, address').ilike('name', 'crispy donuts')
  ...
  url.searchParams.set('panoid', 'HF7DosEYJG3YcK9cDFti0Q')
  url.searchParams.set('yaw', '359.33')
