...
async function main() {
  const { data: shops } = await supabase.from('dd_shops').select('id, name, address').ilike('name', 'texas donuts').ilike('address', '%vine%')
  ...
  url.searchParams.set('pitch', '9.85')
  url.searchParams.set('panoid', 'OGJjiw9LavRPoxcVxFk8oA')
  url.searchParams.set('yaw', '276.19')
