...
const DAY_KEYS: TranslationKey[] = ['hours.sunday', 'hours.monday', 'hours.tuesday', 'hours.wednesday', 'hours.thursday', 'hours.friday', 'hours.saturday']

export default function ShopHours() {
  const { t } = useShopLang()
  const [hours, setHours] = useState(DAY_KEYS.map((_, i) => ({ day_of_week: i, open_time: '06:00', close_time: '18:00', is_closed: false })))
  const [loading, setLoading] = useState(true)
  const [saving, setSaving] = useState(false)

  useEffect(() => {
    fetch('/api/shop/hours').then(r => r.json()).then(data => {
      if (data.length > 0) setHours(DAY_KEYS.map((_, i) => data.find((h: any) => h.day_of_week === i) || { day_of_week: i, open_time: '06:00', close_time: '18:00', is_closed: false }))
      ...
      <div style={{ background: '#fff', borderRadius: 12, border: '1px solid #FFE4EF', overflow: 'hidden' }}>
        {hours.map((h, i) => (
          <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '12px 20px', borderBottom: i < 6 ? '1px solid #FFF0F5' : 'none', opacity: h.is_closed ? 0.5 : 1 }}>
            <span style={{ width: 90, fontWeight: 600, fontSize: 14 }}>{t(DAY_KEYS[i])}</span>
            ...
            <label style={{ fontSize: 12, display: 'flex', alignItems: 'center', gap: 4, marginLeft: 'auto' }}>
              <input type="checkbox" checked={h.is_closed} onChange={e => update(i, 'is_closed', e.target.checked)} /> {t('hours.closed')}
            </label>
          </div>
        ))}
      </div>
