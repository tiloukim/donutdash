...
export default function PayoutsPage() {
  ...
  async function resendSummary(weekStart: string) {
    ...
    // ADMIN_PHONE_NUMBERS plus the email list, so a misclick texts the whole
    // admin group. The button sits next to "mark paid" on the same row,
    // which is exactly where a stray click lands.
    ...
    try {
      ...
      // Formatted defensively — the send already happened by this point, so
      // a missing field must not throw into the catch below and tell the
      // admin it failed when it didn't.
