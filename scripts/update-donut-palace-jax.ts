...
async function main() {
  const { data: shops } = await supabase.from('dd_shops').select('id, name, address, city').ilike('name', 'donut palace').ilike('city', '%jacksonville%')
  ...
  url.searchParams.set('pitch', '0.28')
  url.searchParams.set('panoid', 'fmZ2_MrYcoHj7M9DAJLVUQ')
  url.searchParams.set('yaw', '206.52')
